...
6. **AvatarWindow 桌宠交互（Wheel/Drag + Alt 分流，已验证）**
   - **Wheel**：缩放 *窗口*（以窗口中心为锚点，模型随窗口一起变大/变小）。
   - **Alt+Wheel**：只缩放 *模型*（沿用 VRM scale）。
   - **Left Drag**：拖动 *窗口*（角色一起走）。
   - **Alt+Left Drag**：只拖动 *模型*（窗口不动）。
...
### E. “无论按不按 Alt，Wheel/Drag 都只在操作模型”
- 结论：交互入口在 `canvas.addEventListener(...)`（VRM renderer）里，原实现没有用 Alt 做分流，导致窗口操作逻辑无法生效。
- 处理（收敛后的最终方案）：
  - **把“桌宠交互”收口到 VRM renderer**：默认操作窗口，Alt 才操作模型。
  - **后端 wheel hook 只在 toolMode=avatar 时启用**：解决非激活窗口收不到 wheel 的问题。
...
- Panel 自动隐藏（Windows，全局点外部）  
  - `src-tauri/src/windows/avatar_window.rs`：全局 `WH_MOUSE_LL` 里统一处理 outside-click dismiss
...
- avatar 命中/穿透（cursor gate + mask）  
  - `src-tauri/src/windows/avatar_window.rs`（轮询 gate + `MA_NOACTIVATE`）
- AvatarWindow 桌宠交互（Wheel/Drag + Alt 分流）
  - `src/components/vrm/useVrmRenderer.ts`（交互分流：默认窗口 / Alt 模型）
  - `src-tauri/src/windows/avatar_window.rs`（Windows wheel hook -> emit `avatar-input-wheel`）
  - `src/windows/avatar/AvatarRoot.tsx`（转发 wheel 事件到 canvas；同步 toolMode）
  - `src-tauri/src/commands/avatar_commands.rs`（`avatar_set_tool_mode`）
...
1. 打开 VRM 页，将 Tool 切到 `Avatar`（不是 `Camera`）。
2. 鼠标悬停在角色可交互区域（命中 mask 为 1 的区域）。
3. Wheel：窗口整体缩放（中心锚点），模型随窗口同步变化。
4. Alt+Wheel：仅模型 scale 变化，窗口尺寸不变。
5. Left Drag：窗口移动（角色一起走）。
6. Alt+Left Drag：仅模型位置变化，窗口不动。
